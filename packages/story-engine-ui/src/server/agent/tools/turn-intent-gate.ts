...
// - 否定词表：不/未/别 + 没（豁免 没问题/没毛病/没什么——状况陈述不是推迟，「确认没毛病就入库」
//   与逗号版同判放行，复审 P2-3 误拦回归）+ 无法/无需（「无」不单字扫，防「预览无误后入库」误伤）；
//   「不」不设豁免：不着急/不急着是否定极性的推迟——「我不着急确认入库」「确认不着急入库」拦
//   （复审 P2-b：第四轮「状语豁免」曾把这类推迟句放行真入库）；「没」豁免保住，二者别混；
//   「别」前是「特」豁免（「特别确认」的「别」是语气词的一部分，与确认锚后顾同口径）；
// - 后说话算数：最后一个相关子句（含域内动词，或整句短确认）定结果；
// - 尾句否决：决定性子句之后跟纯否定/推迟尾句（「先别/先等等/算了/慢着/不过我反悔了」）→ 拦（复审 P2-3/P2-c）；
...
const GATE_NEGATION_WORD = /(?<!特)别|没(?!问题|毛病|什么)|无法|无需|不|未/gu;
// 动词后否定：「确认入库不行」「确认提交不了」
...
// 词表按实锤逐条收录、非穷举：新实锤的否决/推迟措辞须连同测试一起进表（复审 P2-c 补 算了/等一下/慢着——
// 「确认入库，算了」「确认定稿，等一下」「确认入库，慢着」此前放行；等等/先等等原有覆盖保持）
const GATE_TRAILING_VETO_CLAUSE =
  /^(?:但|但是|不过|可是)?(?:算了)?(?:我|咱们|你)?(?:现在|目前|暂时|先|再)?(?:别|别急|不行|不好|不着急|不急着|等等|等下|等一等|等一下|缓一缓|慢着|再说|反悔了?|想想|考虑一下|考虑|算了)[。.!！]?$/u;
...
// 纯推迟尾句（词表逐槽对齐 commit 门的 GATE_TRAILING_VETO_CLAUSE，审计返工 B7——draft 表此前漏收
// commit 表的「算了」前缀、咱们/你主语、现在/目前/暂时副词、再说/反悔了?词根）。
// 不能直接复用同一常量：draft 门额外保有三层 commit 表没有的覆盖——「又」副词、「想一下」词根、
// 「再(说|看|想|等等?)」后缀（「想想再说」「等等再说」「考虑一下再说」，P1-4 测试钉住；
// commit 表无此后缀、是其已知洞，勿回退）。
// 注意「你/咱们」只是主语前缀、词根槽仍必填：「你看看行不行」「你说呢」不配词根不否决。
const DRAFT_WRITE_TRAILING_VETO =
  /^(?:但|但是|不过|可是)?(?:算了)?(?:我|咱们|你)?(?:现在|目前|暂时|再|又)?(?:先)?(?:不(?:着急|急着)|别|别急|不行|不好|等等|等下|等一等|等一下|缓一缓|慢着|再说|反悔了?|想想|想一下|考虑一下|考虑|算了)(?:再(?:说|看|想|等等?))?$/u;
